...
import { BusReportUpsertComponent } from './upsert/upsert.component';
...
export class BusReportComponent implements OnInit {
  ...
  add(): void {
    this.modal.createStatic(BusReportUpsertComponent, { i: { id: 0 } }).subscribe(() => this.st.reload());
  }

  edit(item: any): void {
    console.log(item);
    this.modal.createStatic(BusReportUpsertComponent, { record: item, i: { ...item } }, { size: 'md' }).subscribe(() => this.st.reload());
  }
