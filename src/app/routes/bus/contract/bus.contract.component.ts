...
import { BusContractUpsertComponent } from './upsert/upsert.component';

@Component({
  selector: 'app-bus-contract',
  templateUrl: './bus.contract.component.html',
})
export class BusContractComponent implements OnInit {
  args = {};
  body = {};
  ...
  add(): void {
    this.modal.createStatic(BusContractUpsertComponent, { i: { id: 0 } }).subscribe(() => this.st.reload());
  }

  edit(item: any): void {
    console.log(item);
    this.modal.createStatic(BusContractUpsertComponent, { record: item, i: { ...item } }, { size: 'md' }).subscribe(() => this.st.reload());
  }
