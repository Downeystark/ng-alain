...
import { SysRoleUpsertComponent } from './upsert/upsert.component';
...
export class SysRoleComponent implements OnInit {
  ...
  add(): void {
    this.modal.createStatic(SysRoleUpsertComponent, { i: { id: 0 } }).subscribe(() => this.st.reload());
  }

  edit(item: any): void {
    console.log(item);
    this.modal.createStatic(SysRoleUpsertComponent, { record: item, i: { ...item } }, { size: 'md' }).subscribe(() => this.st.reload());
  }
